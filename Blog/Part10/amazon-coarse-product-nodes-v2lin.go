...
// ****************************************************************************
// This is a linearized version of the amazon-coarse-product-nodes-v2.go
// in order to compare processing speed.
// ****************************************************************************
...
func ClusterVectorToFragments(g S.Analytics) S.LinSet {
	...
	var clusters S.LinSet = make(S.LinSet)
	...
	for i := 0; i < len(keys); i++ {
		...
		for j := i + 1; j < len(keys); j++ {
			...
			if d2 < SIMILARITY_THRESHOLD {
				...
				S.LinTogetherWith(clusters,keys[i],keys[j])
			}
		}
...
func ShowHubFractionsForEachCluster(g S.Analytics, clusters S.LinSet) {
