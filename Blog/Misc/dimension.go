...
func main() {
	...
	startnodes := ScatterSamples(g, "Nodes",10)

	for s := range startnodes {
		GetVolumeDistribution(g,"Nodes", startnodes[s], samples, max)
	}
...
func GetVolumeDistribution(g S.Analytics, collection string, startnode string, samples map[int][]float64, max map[string]float64) {
...
func GetAllNodes(g S.Analytics, all_nodes map[string]bool, collection string) {
...
func ScatterSamples(g S.Analytics, collection string, max_samples int) []string {
